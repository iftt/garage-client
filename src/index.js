...
class GarageClient extends FpgaProtocol {
  ...
  constructor(options: Options) {
    ...
    // server
    let origins = options.allowedOrigins | process.env.ORIGINS;
    if (typeof origins === 'string')
      origins = origins.split(',');
    this.socket = new SocketIO(8001, { origins: origins | ['*'] });
    this.setupIO();
    ...
    this.nedDB.findOne({ key: 'services' }, (err, doc) => {
      ...
      this.serviceManager = new ServiceManager(services, this._actions.bind(this));
    });
  ...
  _actions(action: { key: string, value: any }) {
    ...
    if (action.key === 'garageDoor') {
      if (action.value === 1 && this.switchStatus == process.env.GARAGE_OPEN_STATE) // we want the garage to be closed
        changeGarageState = true;
      else if (action.value === 0 && this.switchStatus != process.env.GARAGE_OPEN_STATE) // we want the garage to be open
        changeGarageState = true;
    }
    if (action.key === 'garageDoor' && changeGarageState)
  ...
  program(instructions: [Instructions]) {
    ...
    if (Array.isArray(instructions))
      instructions.forEach(instruction => self.serviceManager.addService(instruction));
  ...
  setupMaM() {
    ...
    self.nedDB.findOne({ key: 'mamstate' }, (err, doc) => {
      ...
      // else
      //   self.mamState = Mam.changeMode(self.mamState, 'restricted', process.env.CLIENT_SECRET); // include encryption :D
      else
  ...
  _watchGarageDoor() {
    ...
    self.reedSwitch.watch((err: null | string, status: number) => {
      if (err)
        console.log(err);
      else if (status !== self.switchStatus) { // watch randomly returns values outside of true changes, and thus can be the same value
        self.switchStatus = status;
        if (status === process.env.GARAGE_OPEN_STATE)
          self.onGarageOpen();
        else
          self.onGarageClose();
      }
